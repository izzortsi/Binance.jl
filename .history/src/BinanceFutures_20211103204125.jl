...
BINANCE_API_WS = "wss://fstream.binance.com:9443/ws/"
...
function getKlines(symbol; startDateTime=nothing, endDateTime=nothing, limit = 500, interval="1m")
    ...
    if startDateTime != nothing && endDateTime != nothing
        ...
        query = string(query, "&startTime=", startTime, "&endTime=", endTime, "&limit=", limit)
    end
...
function wsKlineStreams(callback::Function, symbols::Array; interval="1m")
    #interval => 1m 3m 5m 15m 30m 1h 2h 4h 6h 8h 12h 1d 3d 1w 1M
      allStreams = map(s -> string(lowercase(s), "@kline_", interval), symbols)
      @async begin
        HTTP.WebSockets.open(string("wss://fstream.binance.com:9443/ws/",join(allStreams, "/")); verbose=false) do io
            while !eof(io)
